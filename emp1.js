...
app.get('/addEmp', function(req,res) {
    ...
    let todo = [req.query.first_name,req.query.last_name,req.query.email,req.query.phone_num,req.query.address,req.query.gender,req.query.designation];
    
    let connection = mysql.createConnection(config);
  
    let stmt = `Insert into vishesh.employees(first_name, last_name, email, phone_num, address, gender, designation)
                values(?,?,?,?,?,?,?)`;
...
app.get('/attend1', function(req,res) {
  
  let todo = [req.query.emp_id,curdate(),req.query.val];
  let connection = mysql.createConnection(config);
